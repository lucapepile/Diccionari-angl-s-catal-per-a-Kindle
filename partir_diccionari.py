...
def netejar_divs_repetits(text):
    """Elimina els divs repetits, paràgrafs buits, salts de línia innecessaris"""
    
    # Netegem els <br/> sols que quedin al final
    text = re.sub(r'(<br/>\s*)+$', '', text)

    # Eliminem <div> buits o repetits
    text = re.sub(r'</div>\s*(?=(</div>|<br/>|</idx:entry>))', '', text)
    text = re.sub(r'<div[^>]*>\s*</div>', '', text)

    # Eliminem <p> buits o amb només espais
    text = re.sub(r'<p>\s*</p>', '', text)

    # Eliminem <li> buits
    text = re.sub(r'<li[^>]*>\s*</li>', '', text)

    # Eliminem múltiples <br/> seguits (excepte després de FALSOS AMICS!)
    text = re.sub(r'(<br/>\s*){2,}(?!<span class="accessory_heading">FALSOS AMICS!)', '<br/>', text)

    # Corregim ol aniuats incorrectes
    text = re.sub(r'<ol class="dict"><ol class="dict">', '<ol class="dict">', text)
    text = re.sub(r'</ol></ol>', '</ol>', text)

    # Eliminem <br/> que quedin després d'un tancament de </li> i abans d'un nou <li>
    text = re.sub(r'(</li>\s*)<br/>(\s*<li>)', r'\1\2', text)
    ...
    # Netegem línies en blanc múltiples
    text = re.sub(r'\n\s*\n', '\n', text)

    return text.strip()


def eliminar_vegeu_tambe(entrada):
    """Elimina les seccions 'Vegeu també' i els seus enllaços associats"""
    # Patró per a les seccions "Vegeu també" amb diferents variants d'espaiat i format
    patro_vegeu = re.compile(
        r'(<br/>\s*)+<span class="accessory_heading">Vegeu també:</span><br/>'
        r'(<a [^>]*>.*?</a>\s*)+'
        r'(</ol>\s*)?(</div>\s*)*', 
        flags=re.IGNORECASE
    )
    
    # Eliminem totes les coincidències
    entrada = patro_vegeu.sub('', entrada)
    
    return entrada

def afegir_ol_dict_si_cal(entrada):
    """Encapsula el contingut dins <ol class="dict"><li>...</li></ol> sempre, fins i tot si hi ha una sola accepció"""
    # Busquem el contingut entre <div class="div1"> i el copyright (o final)
    ...
    if definicio_match:
        ...
        if contingut:
            # Si ja hi ha <ol class="dict">, no fem res
            if re.search(r'<ol class="dict">', contingut):
                return entrada
            
            # Si ja hi ha <li>, només encapsula amb <ol>
            if re.search(r'<li\b', contingut):
                nou_contingut = f'{before}<ol class="dict">{contingut}</ol>{after}'
            else:
                # Encapsula encara que només hi hagi una accepció
                nou_contingut = f'{before}<ol class="dict"><li><span></span> {contingut}</li></ol>{after}'

            # Substituïm l'antiga secció pel nou contingut
...
def dividir_diccionari_angles_catala(arxiu_entrada, carpeta_sortida, num_parts=300):
    ...
    capcalera_html = (
        ...
        '<title>Diccionari anglès-català</title>'
        '</head>'
        '<body>'
        '<mbp:frameset>')
    # Títol del diccionari (només per la primera part)
    titol_diccionari = '<h1>Diccionari anglès-català</h1>\n'
    ...
    for part in range(min(num_parts, num_entrades)):
        ...
        with open(nom_fitxer, 'w', encoding='utf-8') as f_sortida:
            ...
            for i in range(inici, fi):
                ...
                # Eliminem les seccions "Vegeu també"
                entrada = eliminar_vegeu_tambe(entrada)
...
if __name__ == "__main__":
    import sys
    nom_arxiu = 'diccionari_angles_catala.html'
    if len(sys.argv) > 1:
        nom_arxiu = sys.argv[1]
    dividir_diccionari_angles_catala(nom_arxiu, 'parts_diccionari')
